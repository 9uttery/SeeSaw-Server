...
public class ChargeController {
    ...
    // 고속충전 조회
    @GetMapping("/api/battery/charge")
    public BaseResponse<ChargeResponseDto> getCharge( @AuthenticationPrincipal UserAccount userAccount) throws BaseException {
        Long userId = userService.resolveUserById(userAccount.getUserId()).getId();

        try {
            ChargeResponseDto res = chargeService.getUserCharge(userId);
