...
public interface ValueRepository extends JpaRepository<Value, Long> {
    ...
    // 사용자 가치 조회
    @Query(value = "select value_id, value_name from ss_value where user_id=:userId and EXTRACT(YEAR FROM created_at) = :year ", nativeQuery = true)
    List<Object[]> getUser3Values(@Param("userId") Long userId, @Param("year") Integer year);
